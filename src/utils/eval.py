...
def plot_precision_recall_curve(precision_dict, recall_dict, model_names, title, ax=None):
    colors = {
        'RandomForest': '#1f77b4',
        'DecisionTree': '#ff7f0e',
        'KNN': '#2ca02c',
...
def plot_roc_curve(fpr_dict, tpr_dict, model_names, title, ax=None):
    colors = {
        'RandomForest': '#1f77b4',
        'DecisionTree': '#ff7f0e',
        'KNN': '#2ca02c',
    ...
    for model_name in model_names:
        ...
        for simplex in hull.simplices[1:]:
            sns.lineplot(x=[fpr_points[j] for j in simplex], y=[tpr_points[j] for j in simplex], alpha=0.5, c=colors[model_name])
